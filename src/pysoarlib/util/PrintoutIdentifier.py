...
class PrintoutIdentifier:
    ...
    def GetAllChildIds(self, attr=None):
        ...
        child_wmes = [
            wme for wme in self.wmes.get(self.root_id, []) if wme[2] in self.wmes
        ]
        if attr is not None:
    ...
    def GetAllChildValues(self, attr=None):
        ...
        child_wmes = [
            wme for wme in self.wmes.get(self.root_id, []) if wme[2] not in self.wmes
        ]
        if attr is not None:
            child_wmes = [wme for wme in child_wmes if wme[1] == attr]
    ...
    def _get_value(self, attr):
        return next(
            (wme[2] for wme in self.wmes.get(self.root_id, []) if wme[1] == attr), None
        )
