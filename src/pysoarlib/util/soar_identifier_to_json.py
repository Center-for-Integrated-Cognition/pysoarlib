...
def soar_identifier_to_json_limited(soar_id, exclusions, depth_limit, current_depth=1):
    json_object = {}
    ...
    for index in range(soar_id.GetNumberChildren()):
        ...
        if value_type == "int":
            ...
            # Convert to identifier and recurse
            child_id = wme.ConvertToIdentifier()
            processed_value = soar_identifier_to_json_limited(child_id, exclusions, depth_limit, current_depth + 1) if current_depth < depth_limit else wme.GetValueAsString()
        else:
            # For other types, get string representation
            processed_value = wme.GetValueAsString()
